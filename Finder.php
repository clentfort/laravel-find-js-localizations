...
class Finder {
  ...
  /**
   * @var Collection $files A list of JavaScript source-files to look for
   *                        translations in
  ...
  protected function findTranslationKeysInJsSourceFiles(Collection $files) 
  {
    ...
    // The process should always return valid JSON unless something went wrong
    // invoking it.
    if (json_last_error() !== JSON_ERROR_NONE) {
      $error = json_last_error_msg();
      throw new RuntimeException(
        "Could not parse JSON returned by node. Failed with error: \"$error\""
      );
    }
  ...
  /**
   * Searches for localization-calls in the configure `$jsSourceDir` and returns
   * a list of found keys grouped by the file they were found in.
   *
   * @return array
   */
